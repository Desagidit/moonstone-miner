...
let cards=[],focus=null,teamIds=[],query={mode:'and',groups:[]},visits=[],visitIndex=-1,savedTroupes=[],activeTroupeId=null,troupeName='New troupe',favouriteIds=new Set(),draggedMemberId=null,factionQuickFilter=false;
...
function renderCards(){
 ...
 const filtered=cards.filter(c=>(!q||[c.name,...c.keywords,...c.custom_tags].join(' ').toLowerCase().includes(q))&&L.matches(c,query,{favourites:favouriteIds})&&(!factionQuickFilter||compatibleWithTroupe(c)));
...
$('add-group').onclick=()=>{query.groups.push({mode:'and',rules:[defaultRule()]});renderFilters();renderCards()};$('clear-filters').onclick=()=>{factionQuickFilter=false;query={mode:'and',groups:[]};$('outer-mode').value='and';renderFilters();renderCards()};$('outer-mode').onchange=()=>{query.mode=$('outer-mode').value;renderCards()};$('find').oninput=renderCards;$('sort').onchange=renderCards;
