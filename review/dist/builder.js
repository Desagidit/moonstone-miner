...
let cards=[],focus=null,teamIds=[],query={mode:'and',groups:[]},factionFilter=false,visits=[],visitIndex=-1,savedTroupes=[],activeTroupeId=null;
...
(async()=>{try{const r=await fetch('/api/cards');if(!r.ok)throw new Error('Could not load cards');cards=(await r.json()).cards;restoreTroupes();$('load-status').textContent=cards.length+' cards';updateFactionFilter();renderFilters();refreshTeam();show(cards.find(c=>c.eligibility.selectable&&L.compatible(c,$('faction').value))||cards[0]);renderCards()}catch(e){$('load-status').textContent=e.message}})();
