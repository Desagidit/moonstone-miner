...
function persist(){const before=clone(savedTroupes),beforeId=activeTroupeId,index=savedTroupes.findIndex(t=>t.id===activeTroupeId);if(index<0&&teamIds.length){activeTroupeId=troupeId();savedTroupes.push({id:activeTroupeId,...troupeSnapshot()});$('saved-troupes').value=activeTroupeId}if(index>=0)savedTroupes[index]={id:activeTroupeId,...troupeSnapshot()};try{localStorage.setItem(storageKey,JSON.stringify({version:2,activeId:activeTroupeId,draft:troupeSnapshot(),troupes:savedTroupes}));try{localStorage.setItem('moonstone-troupe-v1',JSON.stringify(troupeSnapshot()))}catch{}updateStorageControls();return true}catch{savedTroupes=before;activeTroupeId=beforeId;$('storage-status').textContent='Changes could not be saved. Browser storage is unavailable.';return false}}
function restoreTroupes(){try{const saved=JSON.parse(localStorage.getItem(storageKey)||'null');let draft;if(saved?.version===2&&Array.isArray(saved.troupes)){const used=new Set();savedTroupes=saved.troupes.flatMap(t=>{const clean=cleanTroupe(t);if(!clean||typeof t.id!=='string'||used.has(t.id))return [];used.add(t.id);return [{id:t.id,...clean}]});activeTroupeId=savedTroupes.some(t=>t.id===saved.activeId)?saved.activeId:null;draft=cleanTroupe(saved.draft)}else draft=cleanTroupe(JSON.parse(localStorage.getItem('moonstone-troupe-v1')||'null'));if(draft)applyTroupe(draft);if(activeTroupeId||teamIds.length)persist();else updateStorageControls()}catch{$('storage-status').textContent='Saved troupe data could not be read. You can create a new troupe.'}}
...
function renderTroupeChart(){drawTroupeChart(members(),$('troupe-chart'))}
