...
$('new-troupe').onclick=()=>{const answer=globalThis.prompt('Name your new troupe:','');if(answer===null)return;const name=answer.trim().slice(0,100);if(!name){$('notice').textContent='Enter a troupe name.';return}const before=troupeCheckpoint();activeTroupeId=troupeId();applyTroupe({name,ids:[]});savedTroupes.push({id:activeTroupeId,...troupeSnapshot()});$('saved-troupes').value=activeTroupeId;if(!persist()){rollbackTroupe(before,'Browser storage is unavailable. The troupe was not created.');return}$('notice').textContent='';repaintTroupe()};
...
$('rename-troupe').onclick=()=>{const saved=savedTroupes.find(t=>t.id===$('saved-troupes').value);if(!saved)return;const answer=globalThis.prompt('Rename troupe:',saved.name);if(answer===null)return;const name=answer.trim().slice(0,100);if(!name){$('notice').textContent='Enter a troupe name.';return}const before=troupeCheckpoint();saved.name=name;if(saved.id===activeTroupeId)troupeName=name;if(!persist()){rollbackTroupe(before,'Browser storage is unavailable. The troupe was not renamed.');return}$('notice').textContent=''};
