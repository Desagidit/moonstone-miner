...
let cards=[],focus=null,teamIds=[],query={mode:'and',groups:[]},visits=[],visitIndex=-1,savedTroupes=[],activeTroupeId=null,favouriteIds=new Set(),draggedMemberId=null;
...
function troupeSnapshot(){return {name:$('troupe-name').value.trim()||'New troupe',ids:[...teamIds],faction:troupeFaction(),size:6}}
...
function applyTroupe(troupe){teamIds=[...troupe.ids];$('troupe-name').value=troupe.name}
function repaintTroupe(){refreshTeam();renderCards();if(focus)show(focus,false)}
...
$('new-troupe').onclick=()=>{activeTroupeId=null;applyTroupe({name:'New troupe',ids:[],faction:'Undecided',size:6});persist();repaintTroupe()};
...
(async()=>{try{const r=await fetch('/api/cards');if(!r.ok)throw new Error('Could not load cards');cards=(await r.json()).cards;restoreFavourites();restoreTroupes();$('load-status').textContent=cards.length+' cards';renderFilters();refreshTeam();show(cards.find(c=>c.eligibility.selectable&&compatibleWithTroupe(c))||cards[0]);renderCards()}catch(e){$('load-status').textContent=e.message}})();
