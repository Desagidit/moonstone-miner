...
Faction extraction uses `data/faction-symbols.json`, a source-bound map of all 30 visually verified printed symbol image hashes. Split symbols preserve both represented factions; background colour and page-number ranges are not used. Iconless summons retain their associated metadata without becoming recruitable. `python test_factions.py` extracts symbols afresh from all 141 PDF pages and checks every catalogue faction, known mixed-symbol regressions, and unknown/missing-symbol rejection. The global audit corrected Brother Daniel, Dim & Dimmer, Snag, Jobie and Streatham; Knoll’s earlier correction is retained. Partner routes and review verification are regenerated consistently.
