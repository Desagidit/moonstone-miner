...
def export():
 ...
 js=(source/'builder.js').read_text(encoding='utf-8').replace("fetch('/api/cards')","fetch('./data/catalogue.json')").replace("image.src='/'+c.source.image_path","image.src='./'+c.source.image_path").replace("node('a','Review this card ↗')","node('a','Official card ↗')").replace("review.href='/?page='+c.source.pdf_page","review.href=c.source.card_url")
